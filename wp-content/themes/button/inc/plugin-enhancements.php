...
class Button_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Button_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					__( 'The %1$s is required to use some of this theme&rsquo;s features, including: ', 'button' ),
					'<strong>' . __( 'Jetpack plugin', 'button' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => __( 'no specific module needed', 'button' ),
			'custom-content-types' => __( 'Custom Content Types module', 'button' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'button' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'button' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'button' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'button' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'button' ),
...
add_action( 'admin_head', array( 'Button_Theme_Plugin_Enhancements', 'init' ) );
